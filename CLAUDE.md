...
1. `python -m iaq4j` CLI → `iaq4j/__main__.py` → `iaq4j/model_trainer.py` → `training/train.py:train_single_model()` → `training/pipeline.py:TrainingPipeline` — uses `SyntheticSource` by default or `InfluxDBSource`/`CSVDataSource` via `--data-source`
...
**Inference flow**: `SensorReading` → `InferenceEngine` → `IAQPredictor.predict()` → profile feature engineering → scaler transform → torch forward pass → standard clamp/categorize → `IAQResponse`. Uncertainty via MC dropout (MLP/LSTM/CNN), weight sampling (BNN), or history-based fallback (KAN). `InferenceEngine` also supports Bayesian conjugate updates via `prior_variables` (external environmental signals that shift the posterior) and per-engine prediction history for running statistics and sensor drift analysis.
...
- **LLM Readiness (P1)** — Phase 1: config cache invalidation, InfluxDB reads, training exception propagation, standardized error model. Phase 2: REST endpoints for profiles, quantities, models, history, config, training trigger, audit.
- **LLM Agent (P2)** — depends on LLM Readiness Phase 1+2.
